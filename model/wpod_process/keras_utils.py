...
from model.wpod_process.utils import getWH, nms
from model.wpod_process.projection_utils import getRectPts, find_T_matrix
...
def detect_lp(model, I, max_dim, net_step, out_size, threshold):
    ...
    start = time.time()
    Yr = model.predict(T, verbose=False)
    Yr = np.squeeze(Yr)
